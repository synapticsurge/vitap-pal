...
<div class = "flex w-full justify-evenly">
	...
	{#if t1 != "0;"}
	{#if i == 0}
	<button class="btn btn-primary" onclick={async()=>await table1Click(t1)} ><ArrowDownToLine/>Download All</button>
	{:else if i==1}
	<button class="btn btn-primary" onclick={async()=>await table1Click(t1)} ><ArrowDownToLine/> Download General Material</button>
	{:else if i==2}
	<button class="btn btn-primary" onclick={async()=>await table1Click(t1)}  ><ArrowDownToLine/>Download Syllabus</button>
	{/if}
	{/if}	
	{/each}
</div>
		  ...
		  <tbody >
			...
			<tr>
				...
				<th>
					...
					<button onclick={async()=>await table1Click(t2)}><ArrowDownToLine></ArrowDownToLine></button>
					{/each}
				</th>
			  </tr>
			{/each}
