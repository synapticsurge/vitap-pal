...
class LocalDataSource {
  ...
  Future<SemidEntity> getSemidsFromStorage() async {
    List<SemTableData> data = await _globalAsyncQueue.run(
      "get_semids_storage",
      () => _appDatabase.select(_appDatabase.semTable).get(),
    );
      ...
      () => (_appDatabase.batch((batch) {
        ...
        batch.insertAll(_appDatabase.semTable, [
          for (var i in semid.semids)
            SemTableCompanion.insert(semName: i.semName, semid: i.semid),
        ]);
      })),
    );
  }
